...
public class MapModel : Singleton<MapModel>
{
    ...
    public void LoadMaps(int maxId = 0)
    {
        ...
        else
        {
            if (newMaps.Count == 0)
            {
                ...
                selectChapter = ResModel.Instance.config_chapter.GetChapterByMap(mapInfo);
            }
            else
            {
            ...
            {
                selectChapter = (config_chapter_item)ResModel.Instance.config_chapter.data[ResModel.Instance.config_chapter.data.Count - 1];
            }
        }
		...
		if (maxId > 0)
        {
            ...
            passMaps = new List<MapInfo>();
			int openId = 0;
            for (int j = 0; j < ResModel.Instance.config_map.data.Count; j++)
			{
                config_map_item config_map_item = ResModel.Instance.config_map.data[j];
	...
	public void SlideChapter(bool isLeft)
	{
		...
		if (isLeft)
		{
			...
			{
				SwitchChapter((config_chapter_item)ResModel.Instance.config_chapter.GetItemAt(selectChapter.GetIndex()));
			}
		}
		else
		{
			...
			{
				SwitchChapter((config_chapter_item)ResModel.Instance.config_chapter.GetItemAt(selectChapter.GetIndex() - 2));
			}
		}
	}
