...
public class PropDoPart : MonoBehaviour {
	...
	private void Awake()
	{
		list.itemPrefab = ResModel.Instance.GetPrefab("prefab/fightmodule/" + "PropTouchItem");
	...
	public void Show(int propId)
	{
		...
		if(isShow)
		{
			float showWidth = BattleModel.Instance.crtBattle.ShowWidth() * PosUtil.CELL_W * PosUtil.CELL_W_MOD;
			float showOffWidth = BattleModel.Instance.crtBattle.ShowOffsetWidth()*0.5f* PosUtil.CELL_W * PosUtil.CELL_W_MOD;

			float showHeight = BattleModel.Instance.crtBattle.ShowHeight() * PosUtil.CELL_H;
			float showOffHeight = BattleModel.Instance.crtBattle.ShowOffsetHeight()*0.5f* PosUtil.CELL_H;
	...
	private Vector2 CreateBrushItem(int cX, int cY, int oX, int oY,int brushId)
    {
		...
		PosUtil.SetFightCellPos(item.transform, cX + oX, cY - oY);
