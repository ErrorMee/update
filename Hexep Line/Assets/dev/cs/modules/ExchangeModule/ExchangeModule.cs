...
public class ExchangeModule : BaseModule
{
	...
	private void InitView()
	{
		...
		config_wealth_item buyItem = ResModel.Instance.config_wealth.GetItemByType(PlayerModel.Instance.exchangeInfo.type);
		...
		//buyIcon.overrideSprite = ResModel.Instance.GetSprite("dependencies/" + "" + buyItem.icon);
		TIVInfo cost = buyItem.GetPriceList()[0];

        config_wealth_item costItem = (config_wealth_item)ResModel.Instance.config_wealth.GetItem((int)cost.id);
		//costIcon.overrideSprite = ResModel.Instance.GetSprite("dependencies/" + "" + costItem.icon);
	...
	{
		ModuleModel.Instance.RemoveUIModule((int)ModuleEnum.EXCHANGE);
