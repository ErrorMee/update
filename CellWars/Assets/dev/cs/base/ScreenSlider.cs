...
public class ScreenSlider : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            startPos.x = (mouseScreen.x - Screen.width * 0.5f) * PosMgr.GAME_WIDTH / Screen.width;
            startPos.y = (mouseScreen.y - Screen.height * 0.5f) * PosMgr.GAME_HEIGHT / Screen.height;


            if (Math.Abs(startPos.y) > PosMgr.GAME_HEIGHT * 0.33f)
            {
                startValid = false;
            }
            else
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (SlideOpen && startValid && GuildPart.ViewGuilding == false)
            {
                Vector3 mouseScreen = Input.mousePosition;
                Vector2 endPos = new Vector2();
                endPos.x = (mouseScreen.x - Screen.width * 0.5f) * PosMgr.GAME_WIDTH / Screen.width;
                endPos.y = (mouseScreen.y - Screen.height * 0.5f) * PosMgr.GAME_HEIGHT / Screen.height;
