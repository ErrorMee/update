...
public class TaskListPart : MonoBehaviour {
	...
	void Awake()
    {
        itemPrefab = GameMgr.resourceMgr.GetGameObject("prefab/fightmodule.ab", "TaskItem");
    }
	...
	private void ShowTaskProgress(int id, int count,bool isTemp)
    {
		...
		//if(isTemp)
		//{
			//itemCtr.progressText.color = ColorMgr.GetColor(ColorMgr.COLOR_GREEN);
		//}
		//else
		//{
			//itemCtr.progressText.color = ColorMgr.GetColor(ColorMgr.COLOR_FONT_WIGHT);
		//}
    }
	...
	{
		float ax, ay;
		ax = (px) * PosMgr.CELL_W * PosMgr.CELL_W_MOD;
		ay = 80;
